...
let socket, retry = 1000, revision = -1, lastMode = "OFFLINE";

function connect() {
  socket = new WebSocket(webSocketUrl());
  socket.onopen = () => { retry = 1000; if (connection) connection.innerHTML = lastMode === "LIVE" ? '<span class="lamp lamp-red"></span>ON AIR — LIVE' : '<span class="lamp lamp-green"></span>LIVE SYNC'; };
  socket.onmessage = event => {
    let message; try { message = JSON.parse(event.data); } catch { return; }
    const state = message.state; if (!state || Number(state.revision) <= revision) return; revision = Number(state.revision); lastMode = state.mode;
