...
Preview-host identity headers are accepted only when `ENVIRONMENT` is not `production` and `ALLOW_WORKSPACE_AUTH=true`. Production ignores these client-supplied headers and requires a cryptographically valid Cloudflare Access assertion.
...
- The production Studio and API are same-origin; deliberate alternate origins must still pass the exact origin allowlist.
